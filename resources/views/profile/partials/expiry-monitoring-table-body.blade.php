   @forelse ($expiries as $Expirie)
   
                        <tr class="text-text-base hover:bg-gray-100 transition duration-150 ease-in-out">
                            <td class="px-6 py-4 text-text-base">{{ $Expirie->medicine_name ?? 'Unknown' }}</td>
                            <td class="px-6 py-4 text-text-base">{{$Expirie->batch_code ?? 'unknown'}}</td> 
                            <td class="px-6 py-4 text-text-base">{{$Expirie->quantity ?? 'Unknown' }}</td> 
                            ...
                            <td class="px-6 py-4 text-text-base">
                             ...
                             <div class="flex items-center gap-3" x-data="{ showViewModal: false }">
                                ...
                                <button  data-id="{{ $Expirie->id }}" class="delete-btnss bg-red-200 p-1 rounded-sm hover:bg-red-300 transition duration-150 ease-in-out">
                    ...
                    <x-view-modal
                         ...
                         <div class="grid grid-cols-3 md:grid-cols-2 gap-2 ">
                            ...
                            <h1 class="px-6 py-4 text-lg">Supplier name: <span>{{ $Expirie->supplier->supplier_name }}</span></h1>
                        </div>
