   @forelse ($batches as $batch)
                        <tr class="text-text-base hover:bg-gray-100 transition duration-150 ease-in-out">
                            ...
                            <td class="px-6 py-4">
                                @if ($batch->status === 'Valid')
                                    <span class="px-2 py-1 bg-green-200 text-green-800 rounded-md text-sm font-semibold">Valid</span>
                                    
                                @elseif ($batch->status === 'Expired')
                                         <span class="px-2 py-1 bg-red-200 text-green-800 rounded-md text-sm font-semibold">Expired</span>
                                @elseif ($batch->status === 'Out of Stock')
                                        <span class="px-2 py-1 bg-orange-200 text-green-800 rounded-md text-sm font-semibold">Out of Stock</span>
                                @endif
                            </td>
                            <td class="px-6 py-4">
                             <div class="flex items-center gap-3" x-data="{ showEditModal: false }">
                                ...
                                <button  data-id="" class="delete-btn bg-red-200 p-1 rounded-sm hover:bg-red-300 transition duration-150 ease-in-out">
                    ...
                    <div class="mb-4 md:col-span-2">
                        ...
                        <select name="status" required
                            ...
                            <option value="Valid" {{ old('status',$batch->status) == 'Valid' ? 'selected' : '' }}>Valid</option>
